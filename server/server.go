...
//const MessageDelimiter byte = 0x99 // 0b10011001
...
func newServer() *Server {
	server := &Server{
		...
		incoming: make(chan *internalMessage),
...
func (s *Server) Listen() {
	...
	for {
		...
		case data := <-s.incoming:
			log.Println("Got new message: ", data)
			switch data.message.MessageType {
			case AlohaCommand:
				log.Println("Updating username for", *data.ip)
				s.clients[*data.ip].username = data.message.Sender
			case NewMessageCommand:
				log.Println("Broadcasting message: ", data.message, "to", len(s.clients)-1, "users")
				s.outgoing <- *data.message
			case LeaveCommand:
				log.Println("Leave chatroom ", data.ip)
				s.clients[*data.ip].connection.Close()
				delete(s.clients, *data.ip)
			}
		case newConnection := <-s.joins:
			log.Println("New user", newConnection.RemoteAddr())
			s.Join(newConnection)
		}
	}
...
func (s *Server) Join(connection net.Conn) {
	...
	s.clients[connection.RemoteAddr()] = client
}
	...
	for msg := range s.outgoing {
		...
		for _, client := range s.clients {
			if client.username != msg.Sender {
				log.Println("Sending to ", client.ip.String())
				client.writer.WriteString(jsonMessage)
				client.writer.WriteByte(MessageDelimiter)
				client.writer.Flush()
			}
		}
	}
...
func (c *Client) Listen() {
	...
	for {
		line, err := c.reader.ReadString(MessageDelimiter)
		if err != nil {
			// leave command?
			log.Println(err)
			return
		}
		if _, ok := c.server.clients[c.ip]; !ok {
			// if smth go wrong
			return
		}
		...
		// Detected mother hacker
		if msg.Sender != c.server.clients[c.ip].username {
			log.Println("Lol")
			msg.Sender = c.server.clients[c.ip].username
		}
		c.server.incoming <- &internalMessage{ip: &c.ip, message: &msg}
	}
}
